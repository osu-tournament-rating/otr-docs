...
- Fixed the [tournaments list](https://otr.stagec.net/tournaments) search ranking rejected tournaments whose names start with the search text above verified tournaments whose abbreviations match it. Searching now sorts by `Search relevance` unless a sort was chosen first: verified tournaments come first, followed by exact abbreviation, exact name, abbreviation prefix, and name prefix matches. This also applies to `GET /tournaments` with `sort=3` and the Discord bot's `/tournament` search.
