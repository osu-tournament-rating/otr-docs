# Sample Match
This page is dedicated to carrying out calculations to demonstrate an example of rating changes from a single match. We will be following the logic described [[Rating-Calculation#Ranking & rating calcuation|here]], so it is advised to read that page first. This page will focus on detailing the actual calculations used in the [paper](https://jmlr.csail.mit.edu/papers/volume12/weng11a/weng11a.pdf) which introduces this rating model. 
...
> 
> If you are interested more in the general philosophy of what ratings represent, it is highly recommended to read [[Rating-Calculation.md#ranking-rating-calculation|this page]] instead. But if you find this kind of information interesting, please read on.

The sample match we will be using is [LC: (H O L A) vs (Ummm)](https://otr.stagec.xyz/matches/33746) (the osu! match link can also be found [here](https://osu.ppy.sh/community/matches/112506508)). This match was chosen because it is a relatively short match with a smaller format (2v2, team size 3) which still illustrates the main ideas.
...
| [Railgun_](https://osu.ppy.sh/users/13817114) |     $1000$     |            $280$            |
...
In method A of calculating rating changes, we first look at the four players of each game and rank their scores from highest to lowest. The teams that the players play for and the mods that they are irrelevant, except that EZ scores (in this case, parr0t's score in the first game) are multiplied by 1.75. Thus the rankings are as shown:

| Game |   1st    |   2nd    |   3rd    |  4th  |
|:----:|:--------:|:--------:|:--------:|:-----:|
| $1$  | Railgun_ |  parr0t  |  Isita   | Skyy  |
| $2$  |  Isita   |  parr0t  | Railgun_ | Zeer0 |
| $3$  | Railgun_ |  Isita   | poisonvx | Skyy  |
| $4$  |  Isita   | Railgun_ |  parr0t  | Skyy  |
| $5$  |  parr0t  | Railgun_ |  Isita   | Zeer0 |
| $6$  |  Isita   |  parr0t  | Railgun_ | Zeer0 |

These rankings are the only information taken into account for rating calculations (not the exact scores), and for each game, the four players who play it will be given a "game rating change." We'll now calculate the game rating changes for game 1 (calculations for the other games proceed similarly).

Here, we follow the notation and logic of Algorithm 4 of the paper, found on [page 21](https://jmlr.csail.mit.edu/papers/volume12/weng11a/weng11a.pdf) here. (The description also references Algorithm 1, which can be found on page 15.) First, we compute an overall uncertainty constant which the paper calls $c$, which is given by 
$$c = \sqrt{4\beta^2 + \sigma_{\text{Railgun\_}}^2 + \sigma_{\text{parr0t}}^2 + \sigma_{\text{Isita}}^2 + \sigma_{\text{Skyy}}^2} \approx 614.2.$$
Here $\beta = 150$ is a constant specified by our [table of constants](https://github.com/osu-tournament-rating/otr-processor/blob/master/src/model/constants.rs), and the other four terms in the square roots come from the rating deviations of the four players (before the match). This $c$ is used to compute the predicted probabilities of players placing in various orders; roughly, it means that for this game a difference of $c \approx 614.25$ rating points between two players means the higher-rated player has $e \approx 2.7$ times the chance of placing above the lower-rated player.

Next, we calculate (in the notation of the paper) two values $\Omega$ and $\Delta$ for each player in the game, which specify an *additive* rating change and a *multiplicative* rating deviation change. Instead of repeating all of the formulas from the paper, we will try to work out examples in understandable words. 

1. First, consider the highest-ranking player of the game, Railgun_. The model currently thinks the probability that Railgun_ ranked highest is 
   ...
   where we are plugging in the pre-match ratings from our table above. (Notice that this is slightly smaller than 1/4, because Isita's pre-match rating is much higher than Railgun_'s.) Railgun's suggested rating change from this game is then 
   ...
   (For those following closely, the prefactor of $\frac{1}{4}$ is the "variance damping factor" $\gamma_q = \frac{1}{k}$ as discussed on page 26 of the paper.) Notice that the less likely the model thought it was for Railgun_ to place first, and also the higher their rating deviation, the higher their rating gain is.
...
2. Next, consider the second-highest-ranking player, parr0t. The model now cares about the probability of parr0t ranking highest, 
   ...
   as well as the probability of parr0t ranking highest except for Railgun_,
   ...
   Notice that this suggested rating change is smaller than Railgun_'s suggested rating change, since placing second means the formula for $\Omega$ "subtracts off two probabilities instead of one."
   
> [!note]
> The quantity $p_{\text{parr0t 2nd}}$ is *not* the overall probability of parr0t placing 2nd among the four players, but instead the probability that *assuming* Railgun_ placed first, parr0t did better than the remaining players. We use this slightly less precise language for readability.
   
3. Next, to calculate rating changes for the third-highest-ranking player Isita, the model cares about the probability of Isita ranking highest, ranking highest except for Railgun_, and also ranking highest except for Railgun_ and parr0t. The resulting numbers are
   ...
   $$\Omega_{\text{Isita}} = \frac{\sigma_{\text{Isita}}^2}{c} (1 - p_{\text{Isita 1st}} - p_{\text{parr0t 2nd}} -p_{\text{Isita 3rd}}) \approx \boxed{-75.8},$$
   ...
   The most important feature here is that *because Isita was predicted to have a high probability of ranking high, that heavily penalizes their suggested rating change (because those probabilities are subtracted off)*.
...
Remember that all of this was done just for the first game of the match, but the exact same procedure works for all of the other matches too. Here are the resulting values of $c$, $\Omega_i$, and $\Delta_i$ for each one, rounded for readability (if a player did not play in a game, then the corresponding cells are left blank):
...
| :--: | :------: | :------: | :------: | :---: | --------------- |
| $1$  | Railgun_ |  parr0t  |  Isita   | Skyy  | poisonvx, Zeer0 |
| $2$  |  Isita   |  parr0t  | Railgun_ | Zeer0 | poisonvx, Skyy  |
| $3$  | Railgun_ |  Isita   | poisonvx | Skyy  | parr0t, Zeer0   |
| $4$  |  Isita   | Railgun_ |  parr0t  | Skyy  | poisonvx, Zeer0 |
| $5$  |  parr0t  | Railgun_ |  Isita   | Zeer0 | poisonvx, Skyy  |
| $6$  |  Isita   |  parr0t  | Railgun_ | Zeer0 | poisonvx, Skyy  |
...
1. The highest-ranking player, Railgun_, now has 
...
$$\begin{align*}\mu_{\text{parr0t}} &= 1050 + 0.9\left(\frac{60.5 + 55.1 + 0 -41.2 + 99.3 + 55.1}{6}\right) \\
...
$$\sigma_{\text{parr0t}} = 280 \sqrt{1 - \Delta_{\text{eff}}} \approx \boxed{277.7}.$$
...
| [Railgun_](https://osu.ppy.sh/users/13817114) | $1000 \to 1053.4$ |       $280 \to 277.4$       |
...
All players' rating deviations have slightly decreased, indicating that the model is slightly more confident about the updated ratings. And for example, Railgun_'s rating has significantly increased due to their high participation and relatively high placement among players throughout the match. The overall rating changes are roughly but not exactly zero-sum due to the differences in players' rating deviations. 